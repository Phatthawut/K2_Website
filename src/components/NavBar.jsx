	...
	return (
		<header
			className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${
				navBarScrolled ? "bg-blue-800" : "bg-transparent"
			}`}
		>
			{/*desktop nav*/}
			<nav className="hidden lg:flex items-center max-w-[1440px] mx-auto text-white text-2xl px-8 md:px-18">
				...
				<div className="flex flex-1 justify-end gap-2">
					<a href="https://www.facebook.com/k2venture">
						<FaFacebook />
					</a>
					<a href="https://www.instagram.com/k2.venture/">
						<FaInstagram />
					</a>
				</div>
			</nav>
			{/*mobile nav*/}
			<nav
				className={`sticky top-0 lg:hidden bg-blue-800 text-white text-lg px-6 z-40 ${
					navBarScrolled ? "bg-blue-800" : "bg-transparent"
				}`}
			>
			...
			{showScrollTopButton && (
				<button
					onClick={scrollToTop}
					className="fixed bottom-8 right-8 bg-blue-700 text-white p-2 shadow-lg rounded-full hover:bg-blue-700 transition-colors"
				>
					<FaArrowUp size={24} />
				</button>
			)}
